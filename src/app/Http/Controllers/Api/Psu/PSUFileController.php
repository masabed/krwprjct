...
use App\Models\PSUUpload;       // FINAL
use App\Models\PSUUploadTemp;   // TEMP
...
class PSUFileController extends Controller
{
    ...
    /**
     * GET /psu/file/preview/{uuid}
     *
     * Fixed-size preview:
     *  - image/*         → resize ke width=512 (JPEG, aspect ratio dijaga)
     *  - application/pdf → halaman pertama jadi JPEG 512px (Imagick)
     *    - kalau gagal → JSON error singkat (tanpa stack trace)
     *
     * ?source=final|temp|auto (default: auto)
     */
        ...
        if ($isPdf) {
            if (class_exists(\Imagick::class)) {
                try {
                    ...
                    \Log::warning('PSU PDF thumbnail failed', [
                        'file'  => $abs,
                        'error' => $e->getMessage(),
                    ]);
    ...
    /**
     * Helper: cari file di FINAL (PSUUpload) atau TEMP (PSUUploadTemp).
     */
    private function locateFile(string $uuid, string $prefer = 'auto'): ?array
    {
        $prefer  = in_array($prefer, ['final','temp','auto'], true) ? $prefer : 'auto';
        $diskKey = 'local'; // psu_final dan psu_temp ada di disk local (storage/app/private)
        $disk    = Storage::disk($diskKey);

        // FINAL dulu (kecuali prefer=temp)
        if ($prefer !== 'temp') {
            $rec = PSUUpload::where('uuid', $uuid)->first();
        ...
        $tmp = PSUUploadTemp::where('uuid', $uuid)->first();

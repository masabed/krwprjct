...
use App\Models\RutilahuUpload;      // FINAL
use App\Models\RutilahuUploadTemp;  // TEMP
  ...
  public function preview(Request $request, string $uuid)
{
    ...
    $found = $this->locateFile($uuid, $prefer);
    if (!$found) {
        // UUID / file tidak ada di FINAL maupun TEMP → 404
    ...
    $abs    = $found['abs'];      // absolute path di filesystem
    ...
    if ($mime && str_starts_with($mime, 'image/')) {
        if (class_exists(\Intervention\Image\ImageManagerStatic::class)) {
            ...
            } catch (\Throwable $e) { /* fallthrough ke GD */ }
        }
    ...
    if ($isPdf) {
        if (class_exists(\Imagick::class)) {
            try {
                ...
                \Log::warning('RUTILAHU PDF thumbnail failed', [
                    'file'  => $abs,
                    'error' => $e->getMessage(),
                ]);

                // di sisi client tetap dianggap "tidak tersedia" → 404
                return response()->json([
                    'success' => false,
    ...
    private function locateFile(string $uuid, string $prefer = 'auto'): ?array
    {
        ...
        if ($prefer !== 'temp') {
            $rec = RutilahuUpload::where('uuid', $uuid)->first();
        ...
        $tmp = RutilahuUploadTemp::where('uuid', $uuid)->first();
